...
def _bk_fix45_ai_script_crop_profile(script_mode=None):
    ...
    if mode == AI_SCRIPT_PRINT:
        # Druckschrift strikt: kein künstlicher Rand, kein zusätzlicher Kontext.
        # Sonst sieht das lokale Vision-LM bei eng gesetzten Tabellen/Zeilen
        # schnell Text aus der Zeile darüber oder darunter.
        prof["single_pad_x"] = 0
        prof["single_pad_y"] = 0
...
def _bk_fix45_candidate_score(worker, cand: str, kraken: str, page: str, box: str) -> float:
    ...
    score = info
    # Kraken und kompletter LM-Seiten-OCR sind gleichwertig; der Gewinner muss
    # möglichst viele Informationen aus beiden Quellen erhalten.
    for ref, weight in ((kraken, 34.0), (page, 34.0), (box, 12.0)):
        ...
        # Zahlen/Daten dürfen nicht verschwinden.
        nums = _bk_fix45_number_set(r)
        if nums:
            missing_nums = nums - _bk_fix45_number_set(c)
            score -= 18.0 * len(missing_nums)
    # Zu kurze Kandidaten gegenüber Kraken oder Seiten-OCR hart abwerten.
...
def _bk_fix45_merge_candidates(worker, kraken_text: str, page_text: str, box_text: str, prev_final_text: str = "") -> str:
    ...
    # Kandidaten zunächst nicht nach Quelle, sondern nach Vollständigkeit bewerten.
    candidates = []
    ...
    # Wenn eine Quelle eindeutig mehr Inhalt hat, aber die wichtigsten Zahlen der
    # anderen Quelle erhält, darf sie gewinnen. Dadurch werden gekürzte LM-Zeilen
    # abgelehnt, vollständige LM-Seiten-OCR-Zeilen aber akzeptiert.
    candidates.sort(key=lambda x: x[2], reverse=True)
    best_label, best, best_score = candidates[0]

    # Verhindert, dass exakt derselbe Text wie in der Vorzeile irrtümlich erneut
    # übernommen wird, falls Alternativen vorhanden sind.
    try:
...
def _bk_fix45_request_block_reread(self, block_data_url: str, start_idx: int, end_idx: int, current_lines: List[str]) -> List[str]:
    ...
    for i in range(count):
        ...
        # Wenn die 3er-Box-Antwort kürzt, nicht übernehmen. Gegen Kraken UND
        # Seiten-OCR prüfen, weil beide jetzt gleichwertig sind.
        if txt:
...
def _bk_fix45_connect_preview_shortcut(self):
    try:
        ...
        # zusätzlich auf dem Viewport, falls der Fokus dort landet
