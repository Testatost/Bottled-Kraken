...
def _bk_fix37_prev_word_at_column(prev_line: str, pos: int) -> str:
    ...
    if not words:
        return ""
    # Prefer a token that covers the same printed column, otherwise nearest left/right.
...
def _bk_fix37_resolve_ditto_marks_in_lines(lines: List[str]) -> List[str]:
    """Resolve common register ditto marks.

    Important:
    - " or -"- is a repetition mark, not a literal quote.
    - If the OCR attached the mark to a word, e.g. "Beltzkey, the mark is removed.
    - If the mark stands alone, we copy the previous line's token at the same column.
    """
    out: List[str] = []
    prev_line = ""
    ...
    for raw in lines or []:
        ...
        # OCR often merges the mark into the next word: "Beltzkey -> Beltzkey.
        line2 = attached_re.sub("", line2)
        # Also normalize typical -"- fragments that survived because OCR inserted spaces.
