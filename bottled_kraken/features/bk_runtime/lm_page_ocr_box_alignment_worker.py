"""LM-Seiten-OCR mit strikter Abbildung auf vorhandene Overlay-Boxen."""

class BKFullPageLMOCRWithBoxesWorker(BKFullPageLMOCRWorker):
    """LM-Seiten-OCR + vorhandene Overlay-Boxen.

    Ablauf:
    1. komplette Seite per LM lesen,
    2. jede vorhandene Overlay-Box als kurzen Zuordnungsanker lesen,
    3. Seiten-OCR strikt auf die vorhandenen Boxen abbilden.

    Die Anzahl der vorhandenen Overlay-Boxen bleibt unverändert. Es werden keine
    alten Boxen gelöscht und keine zusätzlichen Textzeilen erzeugt.
    """
    ...
    def _fallback_merge_line(self, kraken_text: str, page_text: str, box_text: str, prev_final: str = "") -> str:
        """Fallback nur für fehlende/unklare Seiten-OCR-Zuordnung.

        Für den Modus "LM Seiten OCR + Boxen" ist der Seiten-OCR die Zielquelle.
        Box-/Kraken-Texte dienen nur als Anker. Deshalb bevorzugt dieser Fallback
        page_text vor box_text und kraken_text.
        """
    ...
    def _deterministic_align_page_lines_to_boxes(self, page_lines: List[str], box_lines: List[str]) -> List[str]:
        """Erzeugt exakt eine Ergebniszeile pro vorhandener Overlay-Box.

        Die Seiten-OCR bleibt die bevorzugte Quelle. Box-OCR und bisherige Texte
        dienen nur als Anker, um die passende Seiten-OCR-Zeile zu finden.
        """
        ...
        for i, rv in enumerate(self.recs):
            ...
            for j, candidate in enumerate(page_lines):
                ...
                if anchor:
                    ...
                    score = 0.25
                # Lesereihenfolge als schwache Zusatzinformation nutzen.
