...
def _bk_fix58_xml_text(value) -> str:
    """XML-1.0-sicheren Text erzeugen: Escape + ungültige Steuerzeichen entfernen."""
    text = str(value or "")
    cleaned = []
    for ch in text:
        code = ord(ch)
...
def _bk_fix58_odt_content_xml(blocks) -> str:
    ...
    for block in blocks or []:
        if block.get('type') == 'table':
            ...
            if cols < 2:
                # Keine echte Tabelle: als Absätze schreiben.
                for row in grid:
...
def _bk_fix58_write_odt(path: str, item: TaskItem, export_image: Image.Image, record_views: List[RecordView]):
    ...
    # ODT verlangt: mimetype als erste Datei, unkomprimiert.
...
def _bk_fix58_chunks_from_line(line: str):
    # Trennt nur bei mindestens zwei Leerzeichen. Einfache Leerzeichen innerhalb
    # von Zahlen wie "199 575" oder Namen bleiben erhalten.
    chunks = []
...
def _bk_fix58_cluster_positions(values: List[int]) -> List[int]:
    ...
    clusters = []
    # 3-5 Zeichen Toleranz sind für Monospace-Spalten stabil genug, ohne nahe
    # Tabellenspalten zusammenzuwerfen.
    threshold = 4
    for v in vals:
        if not clusters:
            clusters.append([v])
...
def _bk_fix58_rows_from_spatial_text(lines: List[str]) -> List[List[str]]:
    ...
    for chunks in line_chunks:
        ...
        for start, text in chunks:
            ...
            # Bei Kollision in die nächste leere Spalte schieben, sonst anhängen.
            j = idx
            ...
            if j < len(row) and row[j] == text:
                continue
        # Rechts leere Zellen entfernen, linke Leerspalten erhalten nur dann,
        # wenn in der Zeile danach noch Inhalt folgt.
        while row and not row[-1]:
            row.pop()
        rows.append(row)
    # Vollständig leere Ränder reduzieren.
...
def _bk_fix58_write_csv(path: str, record_views: List[RecordView], image_size=None):
    ...
    with open(path, 'w', newline='', encoding='utf-8-sig') as f:
        # Komma bleibt CSV-Standard; QUOTE_ALL verhindert, dass OCR-Kommas in
        # LibreOffice/Calc als zusätzliche Spalten interpretiert werden.
