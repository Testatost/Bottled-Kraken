...
def _bk_fix43_choose_final_kraken_first(worker, kraken_text: str, page_text: str, box_text: str, prev_final_text: str = '') -> str:
    ...
    # Kraken bleibt Hauptquelle, aber ein längerer Kandidat darf gewinnen, wenn er
    # die Kraken-Information enthält und offenbar nur fehlende Teile ergänzt.
    if kt and not is_suspicious(kt):
        ...
        for cand in (pt, bt):
            ...
            # Kandidaten, die deutlich kürzer sind, werden nie als Verbesserung akzeptiert.
    ...
    # Wenn Kraken leer/suspekt ist, Seiten-OCR als stärkeren Kontext nutzen,
    # Overlay-Box-LM aber nur wenn es nicht gekürzt wirkt.
    best = ""
...
def _bk_fix43_request_block_reread(self, block_data_url: str, start_idx: int, end_idx: int, current_lines: List[str]) -> List[str]:
    ...
    for i in range(count):
        ...
        # Sicherheitsnetz gegen gekürzte Ausgabe.
...
def _bk_fix43_resolve_line_ditto_from_prev(prev_line: str, cur_line: str) -> str:
    ...
    def repl(match):
        ...
        if not repl_text or _bk_fix42_is_ditto_token_text(repl_text) or re.fullmatch(r"[.\s]+", repl_text):
            return ""
        # Nicht denselben Wert unmittelbar doppelt einfügen.
        ...
        if repl_tokens:
            ...
            if last in left_tokens[-2:] or last in right_tokens:
                return ""
        # Mehrere Ditto-Zeichen an praktisch derselben Spaltenposition nur einmal ersetzen.
        pos = match.start()
