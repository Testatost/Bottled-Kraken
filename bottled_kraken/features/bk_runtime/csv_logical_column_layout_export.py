"""Strict standard CSV export for OCR lines.

CSV is a data exchange format, not a page-layout format.  The default CSV export
therefore no longer tries to reconstruct visual page columns, word positions,
OCR boxes, or table geometry.  It writes a small RFC-4180-style line table:

    "line_no","text"
    "0001","Full OCR line text"

Each recognized OCR line becomes exactly one CSV record with exactly two fields.
OCR commas, quotes, and other punctuation stay inside the text field because the
Python csv module performs the required quoting and double-quote escaping.
"""
...
def _bk_fix63_standard_rows(record_views: List[RecordView]):
    """Return the default CSV payload as a plain line table.

    This intentionally has only two columns.  BBox/layout information belongs to
    spatial/overlay exports, not to the normal CSV export opened by users in
    LibreOffice Calc.
    """
    rows = [["line_no", "text"]]
...
# Compatibility aliases for older export hook names and tests.
