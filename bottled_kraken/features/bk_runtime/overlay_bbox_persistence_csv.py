...
try:
    from .main_window_mixins.menu_setup_and_queue_headers import BKStayOpenMenu as _BKStayOpenMenu52
...
def _bk_fix53_adjust_recs_once(task, *, force: bool = False):
    """Apply default bbox scaling (-30 % height, +2 % width) once to new OCR results."""
    try:
...
def _bk_fix53_update_task_preset_bboxes(self, task: TaskItem):
    try:
        # Initial Kraken/blla OCR results often reach the preview without going
        # through _sync_ui_after_recs_change. Scale them here as soon as preset
        # boxes are refreshed.
        ...
        for idx, task in enumerate(getattr(parent, 'queue_items', []) or [], start=1):
            ...
            for v in vals:
                if v and len(v) > 2 and v in s:
                    return idx
        # fallback: currently selected queue row, if any
    ...
    try:
        for task in getattr(parent, 'queue_items', []) or []:
            ...
            for v in sorted(set(vals), key=len, reverse=True):
                if v:
                    out = out.replace(v, '')
        # remove dangling separators left by filename removal
...
try:
    ...
    from .main_window_mixins.menu_setup_and_queue_headers import BKStayOpenMenu as _BKStayOpenMenu53

    def _bk_fix53_cursor_inside_chain_or_menubar(chain):
        try:
            gp = QCursor.pos()
            # Visible menus/submenus.
            for m in chain or []:
            ...
            # Any application top-level menubar under cursor.
    ...
    # Nicht mehr über neue Hover-Logik legen: Das alte Fix53-Verhalten kann
    # Unter-Zusatzfenster zu früh schließen, weil es nur Parent-Widgets prüft.
    # Neue BKStayOpenMenu-Versionen behandeln Menüeinträge, Toolbuttons und
    # sichtbare Untermenüs bereits vollständig selbst.
    if not (
