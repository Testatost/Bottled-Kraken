"""Final reading-order bridge for TXT/ODT/DOCX exports.

This runtime patch only replaces the shared export block model.  OCR, LM
revision, worker dispatch and database/GEDCOM generation are intentionally not
touched.
"""
...
# FIX66: keep indented continuation boxes in their real column without merging
# entries.  The previous attempt merged continuations into one long logical
# line, which collapsed left/right column alignment in TXT and damaged the ODT
# and DOCX column layout.  This narrower fix only changes column assignment:
# boxes that are inside the inter-column bounds stay in that column even when
# they are far from the column's left anchor.  The existing FIX65 line builder
# then keeps them as separate visual lines at their y-position.
    ...
    for r in recs or []:
        ...
        for i in range(len(anchors)):
            if bounds[i] <= x0 < bounds[i + 1]:
                col_idx = i
                break
        # Keep normal left-edge members exactly as before.  For strongly
        # indented sub-lines, allow assignment by column bounds, but only if the
        # box does not cross deep into the next column.  This avoids pulling
        # unrelated full/free text into a column while fixing lines such as
        # "w. Brochaus.", "w. Wagner.", street/name continuations and hyphen
        # continuations that belong under the preceding entry.
...
# Replace only the assignment helper used by FIX65; keep the proven FIX65 block
# ordering and TXT/ODT/DOCX rendering unchanged.
