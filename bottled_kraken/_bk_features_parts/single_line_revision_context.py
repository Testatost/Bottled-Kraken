...
def _bk_fix46_context_excerpt_for_line(rv, page_lines: List[str], max_chars: int = 4200) -> str:
    # Kein hartes Index-Mapping erzwingen: Bei aktueller/markierter Zeile ist die
    # Worker-Zeile lokal nummeriert. Der komplette Seitenkontext ist sicherer.
    ...
    if len(joined) <= max_chars:
        return joined
    # Die sichtbare Zeile, falls möglich, näher im Kontextfenster suchen.
...
def _bk_fix46_request_overlay_box_revision(self, rv, page_context_lines: List[str], local_pos: int, total: int) -> str:
    ...
    # Für die ausgewählte Zeile MUSS die vorhandene Overlay-Box verwendet werden.
    # Ohne Box kann das LM keine saubere zeilenbezogene Korrektur liefern.
    ...
    # Harte Sicherheitsnetze gegen verkürzte Antworten: Die lokale KI darf ergänzen,
    # aber nicht eine bereits erkennbare Kraken-Zeile halbieren.
...
def _bk_fix46_sanity_merge_line(self, kraken_text: str, lm_box_text: str, page_context_text: str, prev_final_text: str = '') -> str:
    ...
    for label, cand in (('lm_box', lt), ('kraken', kt)):
        ...
        # Kraken und LM-Box-OCR sind gleichwertig. Gewinner ist die vollständigere
        # plausible Zeile, nicht starr eine Quelle.
        ...
        if pt:
            # Seitenkontext gibt nur leichte Pluspunkte, weil er nicht exakt ausgerichtet sein muss.
    ...
    best = candidates[0][1]
    # Wenn die LM-Box mehr Information enthält und Kraken-Zahlen erhält, darf LM gewinnen.
...
def _bk_fix46_ai_revision_run(self):
    if isinstance(self, BKFullPageLMOCRWorker):
        # LM-Seiten-OCR selbst unverändert lassen.
        try:
    ...
    try:
        ...
        # 1) Kompletter Seiten-OCR, ohne Overlay-Boxen, nur als Kontext.
        page_lines = _bk_fix46_get_page_context(self)
        page_context_text = '\n'.join(page_lines)

        # 2-5) Jede tatsächlich ausgewählte/übergebene Overlay-Box einzeln senden.
        ...
        # 6) Ditto-Postprocessing positionsbezogen auf den finalen Zeilen.
        try:
...
def _bk_fix48_reassign_target_indices(worker, parent, original_recs):
    """Bewahrt die echten Zeilen-IDs für aktuelle/markierte Zeilen."""
    try:
        rows = []
...
def _bk_fix48_ai_revision_init(self, *args, **kwargs):
    ...
    if parent is None and args:
        try:
            # parent ist in dieser Signatur normalerweise Keyword, dieser Fallback
            # schadet aber nicht bei älteren Zwischenständen.
...
def _bk_fix48_request_true_full_page_ocr_context(self):
    """Echter kompletter Seiten-OCR, nicht nur OCR über die Ziel-Overlay-Boxen."""
    try:
