...
def _bk_fix36_resolve_ditto_marks_in_lines(lines: List[str]) -> List[str]:
    """Resolve register ditto marks using the previous line/column as context."""
    out: List[str] = []
...
def _bk_fix36_table_text_from_recs(recs) -> str:
    """Approximate Origami compose-stage plain-text tables using overlay coordinates."""
    recs = list(recs or [])
    ...
    # Derive global column anchors from x positions.
    anchors = []
    for rv in recs:
    ...
    out = []
    for row in matrix:
        # Keep table-like spatial structure instead of one literary paragraph.
...
def _bk_export_origami_table_docx(self):
    ...
    try:
        ...
        # Fallback: plain text content in a .docx-named file is worse than failing silently;
        # use a sidecar txt when python-docx is unavailable.
