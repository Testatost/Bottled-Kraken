...
def _bk_fix41_normalize_text_value(value) -> str:
    ...
    # Häufige LM-Leaks entfernen: "text: ..." / "bbox_norm: ..." usw.
...
def _bk_fix41_try_parse_obj(raw):
    ...
    if not s:
        return None
    # Original JSON zuerst.
    try:
        return json.loads(s)
    except Exception:
        pass
    # Python-Repräsentation aus versehentlich gecasteten dicts/listen.
    try:
        return _bk_fix41_ast.literal_eval(s)
    except Exception:
        pass
    # Dict-Repr in einer längeren Zeile suchen.
...
def _bk_fix41_extract_lm_page_items(payload) -> List[Dict[str, Any]]:
    ...
    if isinstance(payload, (dict, list)):
        ...
        if obj is not None:
            ...
            # JSON-ish Objektblöcke extrahieren.
    ...
    # Mehrzeilige Textfelder als einzelne Zeilen splitten und eine vorhandene Box verteilen.
...
def _bk_fix41_extract_prev_place(prev: str) -> str:
    s = str(prev or '')
    # take text between last year/date and trailing register number.
    m = re.search(r'(?:1[5-9]\d{2}|20\d{2})\s*[.,;:]?\s*([^\d]{2,50}?)(?:\s*\d{1,4}\s*[.,;:]?\s*)?$', s)
    if m:
        cand = re.sub(r'["„“”]', '', m.group(1)).strip(' .,;:-')
        if re.search(r'[A-Za-zÄÖÜäöüß]', cand):
            return cand
    # fallback: last capitalized word/phrase before final number.
...
def _bk_fix41_resolve_ditto_marks_in_lines(lines: List[str]) -> List[str]:
    ...
    for raw in lines or []:
        ...
        def repl_attached(match):
            ...
            # If the mark is immediately before a place/name, it often repeats the previous year/date column.
        ...
        def repl(match):
            ...
            # Context-aware guesses for common register tables.
            ...
            for rv in row:
                ...
                if re.fullmatch(r'\s*(?:[-–—]\s*)?["„“”]{1,4}(?:\s*[-–—])?\s*', txt):
                    if prev_row and getattr(rv, 'bbox', None):
                        ...
                        if candidates:
                            # Same visual position, not last word of previous line.
...
def _bk_fix41_ai_revision_run(self):
    # Keep the dedicated full-page OCR worker untouched; this only affects line revision worker instances.
    ...
    try:
        ...
        # 1) Full page OCR only as context. It is weak context, not the source of truth.
        ...
        # 2) Send exactly 3 overlay boxes at a time.
        ...
        for i, rv in enumerate(self.recs):
            ...
            # Ask LM decision only if Kraken is empty/suspicious; otherwise Kraken remains the source of truth.
...
def _bk_fix41_render_file(self, path: str, fmt: str, item: TaskItem):
    ...
    if fmt_l == 'docx':
        # Keep the stricter table-detection docx writer from FIX8.40.
