...
def _bk_canonical_extract_place_candidates(line: str) -> List[str]:
    places = []
    # Common pattern in register indexes: "... 1766.Borschim. 155" or "... 1725, Rogelitz."
    for m in re.finditer(r"(?:1[5-9]\d{2}|20\d{2})[.,;:\s]*([A-ZÄÖÜ][A-Za-zÄÖÜäöüß][A-Za-zÄÖÜäöüß\-.]{1,}(?:\s+[A-ZÄÖÜ][A-Za-zÄÖÜäöüß\-.]{2,}){0,2})", line):
        ...
        if 2 <= len(cand) <= 60 and not re.match(r"^\d+$", cand) and cand not in places:
            places.append(cand)
    # Fallback: final capitalized word before a terminal record/page number.
...
def _bk_canonical_extract_person_candidate(line: str) -> Optional[Dict[str, Any]]:
    ...
    # Prefer text before age/date/year punctuation.
    ...
    if len(tokens) < 2:
        return None
    # Limit overly long OCR fragments.
    tokens = tokens[:4]
    ...
    # Genealogical/register pages often list surname first: "Böhm Johann Georg".
    last_name = tokens[0]
...
def _bk_build_canonical_json_fallback_from_text(source_text: str, reason: Any = None) -> Dict[str, Any]:
    """Create a conservative canonical JSON locally when the LM returns prose or invalid JSON.

    This prevents the Canonical Graph workflow from failing completely when a local
    model ignores "JSON only" instructions. The fallback is intentionally conservative:
    it extracts likely person labels, years and places from line-level OCR text and
    builds weak/medium-confidence edges from co-occurrence within a line.
    """
    ...
    for line_idx, line in enumerate(lines[:500], start=1):
        ...
        # Link co-occurring years and places weakly when no person was found.
        if not person_id:
...
def _bk_build_canonical_json(self) -> Dict[str, Any]:
    ...
    # One shorter retry. Some local models fail when the OCR text is very long.
    retry_prompt = (
    ...
    # Do not abort the graph workflow just because the local model returned prose.
    # Build a conservative local canonical JSON so the user can still inspect a graph.
