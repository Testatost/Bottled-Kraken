...
def _bk_fix40_extract_jsonish_lines(payload) -> List[Dict[str, Any]]:
    """Parse valid JSON and common JSON-ish LM output.

    The LM sometimes returns keys without quotes or lets the raw JSON leak into the
    visible OCR list. This parser extracts only actual text/bbox pairs.
    """
    if isinstance(payload, list):
        ...
        try:
            ...
            data = None
        if data is None:
            # Extract object-like blocks if possible.
            ...
            if items:
                ...
                # Last fallback: keep only readable lines, drop JSON syntax/bbox debris.
                lines = []
    ...
    for idx, item in enumerate(data or []):
        ...
        # If one item contains multiple lines and one bbox, split that bbox vertically.
        if len(parts) > 1:
