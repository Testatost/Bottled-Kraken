"""Runtime-Anbindung der einstellbaren LM-Verhaltensoptionen.

Diese Datei verbindet die Beta-LM-Optionen mit dem alten, funktionierenden
Zeilen-Scope. Sie darf keine Worker-Run-Logik und keine Zielzeilen-Dispatch-Logik
ersetzen. Erlaubt sind nur:
- Verhalten/Script-Modus am Worker speichern,
- Seitenkontext je Scope aktivieren/deaktivieren,
- Overlay-Crop/Prompt/Filter aus den LM-Optionen verwenden,
- die bestehende alte Merge-Funktion verhaltensabhängig delegieren.
"""
...
def _bk_lm_behavior_revision_mode(behavior: dict) -> str:
    # Der aktuell normalisierte Dialogwert MUSS Vorrang vor alten Alias-Keys
    # haben.  Sonst kann ein altes revision_mode=kraken_lm_revision eine neu
    # gewählte weight=lm_first Einstellung wieder überstimmen.
...
def _bk_lm_behavior_filter_config(behavior: dict) -> str:
    # Runtime alias bridge for editierbare Filter: old key "filters",
    # contract/UI key "filter_text".
...
def _bk_lm_behavior_request_overlay_box_revision(self, rv, page_context_lines: List[str], local_pos: int, total: int) -> str:
    ...
    if _bk_lm_behavior_is_json_debris(text) or "\n" in str(text or ""):
        text = ""
    if mode == "lm_first":
        # Bei LM-OCR > Kraken-OCR darf hier kein stiller Kraken-Ersatz als
        # angebliches LM-Ergebnis zurücklaufen.  Der Merge entscheidet dann erst
        # am Ende über den Fallback.
        return text
...
def _bk_lm_behavior_request_source_revision(worker, kraken_text: str, lm_text: str, page_text: str = '') -> str:
    ...
    try:
        ...
        if revised:
            # Schutz gegen Umschreiben ganzer Zeilen: Wenn die Antwort massiv kuerzer
            # oder fast ohne Tokenueberlappung ist, bleibt die Quelle erhalten.
            try:
        ...
        try:
            ...
            pass
    # Ohne zusaetzlichen Revision-Call bleibt die Quelle stabil; einzelne harte
    # Fallbacks vermeidet das bisherige versehentliche Uebernehmen ganzer LM-Zeilen.
    return kt
...
def _bk_lm_behavior_sanity_merge_line(worker, kraken_text: str, lm_box_text: str, page_line_text: str = "", prev_final_text: str = "", full_page_context: str = "", page_index_aligned: bool = True) -> str:
    ...
    if mode == "lm_first":
        # LM-OCR > Kraken-OCR: Das echte zeilenbezogene LM-OCR-Ergebnis der
        # Overlay-Box gewinnt. Kraken ist nur Fallback, wenn keine brauchbare
        # LM-Zeile vorhanden ist.
        return lm_visible or page_visible or kt

    if mode == "kraken_first":
        # Kraken-OCR > LM-OCR: Kein hartes Kraken-Festhalten mehr.  Beide
        # Kandidaten werden an den LM-Sanity-Check gegeben; das LM entscheidet,
        # welche Zeile besser ist.
        return _bk_lm_behavior_request_sanity_choice(worker, kt, lm_visible or lt, page_visible or pt)

    if mode == "kraken_lm_revision":
        # Kraken-OCR (Quelle) ueberarbeitet durch LM: Quelle bleibt Kraken; das
        # LM darf nur einzelne offensichtliche OCR-Wortfehler korrigieren.
