...
class HFDownloadWorker(QThread):
    ...
    def _run_simple_command(self, cmd: List[str], status_text: str):
        ...
        ret = self._proc.wait()
        if ret != 0:
            raise RuntimeError(self._tr('hf_error_command_exit', ret, ' '.join(cmd)))
        ...
        try:
            ...
            while True:
                ...
                if now - last_emit_ts >= 0.25:
                    ...
                    lines = [
                        self._tr("hf_status_size_eta", percent_tenths / 10, elapsed)
                    ]
                    if total_files > 0:
                    ...
                    lines = [
                        self._tr("hf_status_download_speed", downloaded_mb, elapsed, speed)
                    ]
                    if total_files > 0:
                        lines.append(self._tr("hf_status_files_done", finished_files, total_files))
                    if self._current_file:
