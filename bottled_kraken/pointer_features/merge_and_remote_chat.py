...
def _ptr_remote_chat_completion(config: PtrRemoteAIConfig, messages: List[Dict[str, str]],
    ...
    try:
        ...
        try:
            ...
            body = str(exc)
        raise RuntimeError(_ptr_text("ptr_err_http_generic", exc.code, body)) from exc
    except urllib.error.URLError as exc:
        raise RuntimeError(_ptr_text("ptr_err_request_failed", exc)) from exc
    except socket.timeout as exc:
        raise RuntimeError(_ptr_text("ptr_err_request_timeout")) from exc
    try:
        data = json.loads(raw)
    except Exception as exc:
        raise RuntimeError(_ptr_text("ptr_err_invalid_json_response")) from exc
    if not isinstance(data, dict):
        raise RuntimeError(_ptr_text("ptr_err_unexpected_response"))
    return data
def _ptr_extract_content_from_chat_response(data: Dict[str, Any]) -> str:
    choices = data.get("choices")
    if not isinstance(choices, list) or not choices:
        raise RuntimeError(_ptr_text("ptr_err_no_choices"))
    first_choice = choices[0]
    if not isinstance(first_choice, dict):
        raise RuntimeError(_ptr_text("ptr_err_choice_invalid"))
    message = first_choice.get("message")
    if not isinstance(message, dict):
        raise RuntimeError(_ptr_text("ptr_err_message_invalid"))
    ...
    if isinstance(reasoning, str) and reasoning.strip():
        ...
        if str(finish_reason).lower() == "length":
            raise RuntimeError(_ptr_text("ptr_err_reasoning_truncated"))
        raise RuntimeError(_ptr_text("ptr_err_reasoning_only"))
    raise RuntimeError(_ptr_text("ptr_err_response_empty"))
__all__ = [
    '_ptr_build_merge_input_text',
    '_ptr_extract_content_from_chat_response',
    '_ptr_extract_json_object',
    '_ptr_merge_ocr_texts_local',
    '_ptr_normalize_line',
    '_ptr_pick_best_line',
