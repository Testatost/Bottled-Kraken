...
def _ptr_on_multi_file_error(self, path: str, message: str):
    ...
    if task:
        ...
        self._update_queue_row(path)
    self.status_bar.showMessage(self._tr("ptr_multi_status_error_short", os.path.basename(path)), 3000)
    self._log(self._tr("ptr_multi_status_error_short", os.path.basename(path)) + f" -> {message}")
...
def _ptr_on_multi_failed(self, message: str):
    ...
    if worker is not None:
        try:
            ...
            pass
    QMessageBox.critical(self, self._tr("ptr_multi_followup_title"), str(message))
...
def _ptr_queue_context_menu(self, pos):
    ...
    multi_ocr_act = menu.addAction(self._tr("ptr_multi_ocr_btn"))
    ai_revise_act = menu.addAction(self._tr("act_ai_revise"))
    ai_tools_act = menu.addAction(self._tr("ptr_ai_tools_title"))
    menu.addSeparator()
    export_merge_act = menu.addAction(self._tr("ptr_export_ai_merge"))
    export_pg_act = menu.addAction(self._tr("ptr_export_postgres_json"))
    export_neo_act = menu.addAction(self._tr("ptr_export_neo4j_json"))
    open_followup_act = menu.addAction(self._tr("ptr_ai_followup_open"))
...
__all__ = [
    ...
    '_old_mainwindow_init',
    '_old_mainwindow_retranslate_ui',
    '_old_mainwindow_stop_ocr',
    '_ptr_mainwindow_all_workers_wrapper',
    '_ptr_mainwindow_init_wrapper',
    '_ptr_mainwindow_retranslate_ui_wrapper',
    '_ptr_mainwindow_stop_ocr_wrapper',
    '_ptr_normalize_remote_base_url',
    '_ptr_on_multi_batch_finished',
    '_ptr_on_multi_failed',
    '_ptr_on_multi_file_done',
    '_ptr_on_multi_file_error',
    '_ptr_on_multi_file_started',
    '_ptr_open_ai_tools_for_current_task',
    '_ptr_queue_context_menu',
    '_ptr_ui_lang',
    '_ptr_ui_tr',
]
