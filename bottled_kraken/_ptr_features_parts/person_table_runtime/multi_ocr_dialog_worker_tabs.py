"""Dialog and worker hooks for OCR variant tabs."""
...
def _ptr_multi_normalize_page_image(im):
    """Normalize input so every preprocessing variant keeps the same size/mode."""
    try:
...
def _ptr_multi_otsu_threshold(gray):
    """Compute a simple Otsu threshold without adding another dependency."""
    try:
        hist = gray.histogram()
        total = sum(hist)
        if total <= 0:
...
def _ptr_multi_build_selected_image_variants(im, variant_keys, job=None):
    """Build deterministic same-size page variants for Kraken recognition."""
