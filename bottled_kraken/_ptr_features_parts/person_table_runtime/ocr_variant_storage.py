"""Eigenständige OCR-Reiter-Zustände pro Queue-Eintrag.

Jeder Reiter ist ein eigener OCR-Zustand. Der aktive Zustand wird in das
normale Datenmodell ``task.results`` geladen, damit alle bestehenden Aktionen
(LM-Überarbeitung, OpenRouter, Overlay-Bearbeitung, Löschen, Skalierung usw.)
ohne Sonderpfad auf dem aktuell sichtbaren Reiter arbeiten.
"""
...
def _ptr_ensure_stores(self):
    ...
    if not isinstance(canonical, dict):
        canonical = {}
    # If older Multi-OCR code already stored real variants before the canonical
    # store was created, preserve them instead of replacing them with an empty
    # canonical dict.
