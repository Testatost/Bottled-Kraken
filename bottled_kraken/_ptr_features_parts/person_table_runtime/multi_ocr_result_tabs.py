"""OCR-Varianten-Reiter für normale und Multi-OCR-Ergebnisse.

Jeder Reiter hält einen eigenen Zustand aus Textzeilen und Overlay-Boxen. Der
aktive Reiter wird immer in die normale Zeilentabelle und in den normalen Canvas
geladen. Dadurch arbeiten alle bestehenden Funktionen weiterhin auf genau dem
sichtbaren Reiter: LM-Überarbeitung, OpenRouter, Overlay-Bearbeitung,
Zeilenlöschen, Zeilenimport, Overlay-Skalierung usw.
"""

try:
    from PySide6.QtCore import Qt
    from PySide6.QtWidgets import QAbstractItemView, QTabBar
except Exception:  # pragma: no cover - PySide ist zur Laufzeit verfügbar.
    Qt = None
    QAbstractItemView = None
    QTabBar = None

try:
    from bottled_kraken.main_window_mixins.ocr_variant_tabs import configure_ocr_variant_tab_buttons
...
def _ptr_ensure_tabs(self):
    ...
    # Prefer the permanent tab bar created by the normal UI builder.  Earlier
    # versions created this bar only from the runtime hook, which could fail
    # silently depending on import order/layout timing.
