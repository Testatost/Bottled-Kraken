...
class ExportWorker(QThread):
    ...
    file_done = Signal(str, str, int, int)
    file_error = Signal(str, str, int, int)
    progress_changed = Signal(int)
    status_changed = Signal(str)
    finished_batch = Signal()
    def __init__(self, render_callback, items: List[TaskItem], fmt: str, folder: str, tr_func=None, parent=None):
    ...
    def run(self):
        ...
        for i, it in enumerate(self.items, start=1):
            ...
            self.status_changed.emit(self._tr("export_status_file", i, total, it.display_name))
            try:
...
class CombinedPDFExportWorker(QThread):
    page_started = Signal(str, int, int)
    pdf_done = Signal(str, int)
    export_error = Signal(str)
    cancelled_export = Signal(str)
    progress_changed = Signal(int)
    status_changed = Signal(str)
    finished_batch = Signal()
