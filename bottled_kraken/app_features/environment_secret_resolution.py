    ...
    try:
        ...
        if exc.code == 401 and is_openrouter:
            ...
            if api_key_source and api_key_source != 'dialog':
                source_hint = _ptr_ui_tr(self, 'ptr_err_http_401_source_hint', api_key_source)
            raise RuntimeError(
                _ptr_ui_tr(self, 'ptr_err_http_401_openrouter', source_hint, body_clean)
            ) from exc
        raise RuntimeError(_ptr_ui_tr(self, 'ptr_err_http_generic', exc.code, body_clean)) from exc
    except urllib.error.URLError as exc:
        raise RuntimeError(_ptr_mask_sensitive_text_v23(_ptr_ui_tr(self, 'ptr_err_request_failed', exc), [resolved_api_key])) from exc
    except socket.timeout as exc:
        raise RuntimeError(_ptr_ui_tr(self, 'ptr_err_request_timeout')) from exc
    try:
        data = json.loads(raw)
    except Exception as exc:
        raise RuntimeError(_ptr_ui_tr(self, 'ptr_err_invalid_json_response')) from exc
    if not isinstance(data, dict):
        raise RuntimeError(_ptr_ui_tr(self, 'ptr_err_unexpected_response'))
    return data
...
__all__ = [
    ...
    '_DOTENV_LINE_RE_V23',
    '_bk_patch24_lang',
    '_bk_patch24_prev_help_theme_values',
    '_bk_patch24_tr',
    '_bk_patch24_translate_error_message',
    '_help_theme_values',
    '_help_theme_values_v24',
    '_ptr_ai_dialog_apply_key_hints_v23',
    '_ptr_ai_dialog_build_ui_v3',
    '_ptr_ai_dialog_set_config_v3',
    '_ptr_ai_dialog_start_worker_v24',
    '_ptr_candidate_api_env_names_v23',
    '_ptr_lookup_api_key_from_env_v23',
    '_ptr_mask_sensitive_text_v23',
    '_ptr_read_dotenv_file_v23',
    '_ptr_remote_ai_config_repr_v23',
    '_ptr_remote_chat_completion',
    '_ptr_remote_chat_completion_v3',
    '_ptr_resolve_remote_api_key_v23',
]
