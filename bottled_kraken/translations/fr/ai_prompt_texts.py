...
FR_AI_PROMPT_TEXTS_TRANSLATIONS = {'ai_prompt_page_system': 'Du bist ein hochpräziser OCR- und Transkriptionsassistent für historische deutsche Drucke, Handschriften und Formulare.\n'
                          'Du liest den Text direkt aus dem Bild.\n'
                          'Das Bild ist die einzige Wahrheitsquelle.\n'
                          'IMPORTANT : les guillemets doubles (") et les séquences comme -"- dans les registres sont souvent des marques de répétition. Ne les '
                          'traite pas littéralement ; remplace-les par la valeur correspondante de la ligne précédente ou de la même colonne dans l’entrée '
                          'précédente.\n'
                          'Un guillemet simple (") dans des tableaux ou registres signifie généralement : répéter la valeur correspondante de la ligne '
                          'précédente ; résous ces marques de répétition à partir du contexte.\n'
 ...
 'ai_prompt_single_system': 'Du bist ein präziser OCR- und Transkriptionsassistent für historische deutsche Handschriften und Formulare.\n'
                            ...
                            'Das Bild ist die einzige Wahrheitsquelle.\n'
                            'IMPORTANT : les guillemets doubles (") et les séquences comme -"- dans les registres sont souvent des marques de répétition. Ne '
                            'les traite pas littéralement ; remplace-les par la valeur correspondante de la ligne précédente ou de la même colonne dans '
                            'l’entrée précédente.\n'
                            'Un guillemet simple (") dans des tableaux ou registres signifie généralement : répéter la valeur correspondante de la ligne '
                            'précédente ; résous ces marques de répétition à partir du contexte.\n'
...
# PATCH49_LM_REVISION_SANITY_PROMPTS_FR_AI_PROMPT_TEXTS_TRANSLATIONS
FR_AI_PROMPT_TEXTS_TRANSLATIONS.update({'ai_prompt_decision_system': 'Tu es un assistant précis de correction OCR pour écritures historiques, textes imprimés, tableaux et formulaires.\nTu reçois trois candidats pour exactement une ligne cible :\n1. le texte Kraken/de ligne précédent\n2. l’OCR du contexte local de page/bloc\n3. l’OCR de la boîte de superposition de cette ligne\n\nTâche :\n- Produis la meilleure version corrigée pour exactement cette ligne cible.\n- Kraken reste l’ancre conservatrice et ne doit pas être remplacé sans raison.\n- L’OCR LM peut améliorer Kraken si elle ajoute de manière plausible des mots, caractères ou espaces manquants, ou corrige des lectures clairement fausses.\n- Aucun nom, lieu, date, âge, année ou nombre d’un texte Kraken plausible ne doit disparaître.\n- Utilise le contexte de page/bloc seulement s’il correspond à la ligne cible ; ne copie jamais les lignes voisines.\n- Conserve l’orthographe historique ainsi que les noms et nombres originaux.\n- N’invente aucune information.\n- Ne retourne jamais plus d’une ligne.\nRéponds uniquement avec du JSON valide.\nPas de Markdown. Pas de texte supplémentaire. Pas de commentaire.\nIMPORTANT : Les guillemets doubles (") et les séquences comme -"- sont souvent des marques de répétition dans les registres. Ne les rends pas littéralement ; remplace-les par la valeur correspondante de la ligne précédente ou de la même colonne dans l’entrée précédente.', 'ai_prompt_decision_user': 'Ligne cible idx={}\n\nTexte Kraken/de ligne précédent :\n{}\n\nOCR du contexte page/bloc :\n{}\n\nOCR de la boîte de superposition :\n{}\n\nCompare les trois candidats et retourne la meilleure version corrigée pour EXACTEMENT cette ligne cible.\nLe texte cible peut différer du texte Kraken précédent si l’OCR LM passe le contrôle de cohérence et donne une lecture plus complète ou plus claire.\nPas de ligne voisine, pas d’explication, pas de Markdown.\nFormat exact :\n{{"text":"..."}}'})
