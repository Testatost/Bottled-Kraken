...
# coremltools can emit a harmless SyntaxWarning while Kraken is imported.
# Install the filter before any Kraken/coremltools import happens.
...
# existing GUI workflow and result handling stay unchanged while running on 7.0.2.
...
def load_kraken_recognition_model(path: str, device: Any = None):
    """Load a Kraken recognition model with Kraken 7.0.2-compatible arguments."""
...
def load_kraken_segmentation_model(path: str, device: Any = None):
    ...
    # Kraken 7.0.2's TorchVGSLModel.load_model no longer takes a device keyword.
