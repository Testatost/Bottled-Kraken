...
def _apply_ocr_auto_revision_replacements(text: Any, replacements=None) -> str:
    ...
    for src, dst in _parse_ocr_auto_revision_replacements(replacements): txt = txt.replace(src, dst)
...
def _clean_ocr_raw_text(text: Any) -> str:
    """Conservative cleanup for direct Kraken-OCR display.

    This removes invisible/control whitespace, but deliberately does not
    normalize historical/typographic OCR characters. Use this when the user
    wants the raw Kraken transcription without automatic smoothing.
    """
    if text is None:
        return ""
    if isinstance(text, bytes):
...
def _clean_ocr_text_for_kraken_display(text: Any, auto_revision_enabled: bool = False, auto_revision_replacements=None) -> str:
    """Cleanup used only for direct Kraken-OCR results.

    Automatic revision is user-configurable in Kraken options. Disabled mode
    keeps historical/typographic OCR characters and only removes invisible
    characters and repeated whitespace. Enabled mode applies the configured
    replacement list.
    """
...
def _ai_script_crop_profile(script_mode: Optional[str]) -> Dict[str, int]:
    ...
    return {
        # Druckschrift ist bereits klar getrennt. Für diesen Modus darf die
        # LM-Zeilenüberarbeitung keine Nachbarzeilen durch künstlichen Rand sehen.
        "single_pad_x": 0,
        "single_pad_y": 0,
