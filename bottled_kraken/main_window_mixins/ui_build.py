...
class MainWindowUiBuildMixin:
    ...
    def _init_ui(self):
        ...
        queue_head.addWidget(self.btn_delete_checked_queue, 0, Qt.AlignRight)
        ...
        import_menu = BKStayOpenMenu(self)
        ...
        inline_search_layout = QHBoxLayout(self.line_search_inline_panel)
        inline_search_layout.setContentsMargins(0, 0, 0, 0)
        inline_search_layout.setSpacing(0)
        ...
        inline_search_layout.addWidget(self.line_search_popup_edit, 0, Qt.AlignLeft)
        inline_search_layout.addStretch(1)
        self.line_search_popup = self.line_search_inline_panel
        self.line_search_inline_panel.hide()
        ...
        lines_head.addWidget(self.line_search_button_panel, 0, Qt.AlignTop)
        ...
        self.ocr_variant_tabs.setMovable(False)
        self.ocr_variant_tabs.setDrawBase(True)
        try:
            self.ocr_variant_tabs.setUsesScrollButtons(False)
        except Exception:
            pass
        try:
            first_label = ocr_variant_tab_label(self, 0)
            first_index = self.ocr_variant_tabs.addTab(ocr_variant_tab_display_text(first_label))
            self.ocr_variant_tabs.setTabToolTip(first_index, first_label)
            plus_index = self.ocr_variant_tabs.addTab("+")
            self.ocr_variant_tabs.setTabToolTip(plus_index, self._tr("multi_ocr_variant_add_tooltip"))
        except Exception:
            self.ocr_variant_tabs.addTab(ocr_variant_tab_display_text("OCR (1)"))
            self.ocr_variant_tabs.addTab("+")
        self._refresh_ocr_variant_tab_texts = lambda: refresh_ocr_variant_tab_texts(self)
        try:
            self.ocr_variant_tabs.tabBarClicked.connect(lambda idx: on_ocr_variant_tab_clicked(self, idx))
            self.ocr_variant_tabs.currentChanged.connect(lambda idx: on_ocr_variant_current_changed(self, idx))
            self.ocr_variant_tabs.tabCloseRequested.connect(lambda idx: delete_ocr_variant_tab(self, idx))
